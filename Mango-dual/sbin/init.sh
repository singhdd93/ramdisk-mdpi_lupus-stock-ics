...
elif [ -s /dev/keycheck2 -o -e /cache/recovery/boot2 ];
	then
		busybox echo "CHOOSING CWM" >> boot.txt
		busybox rm -fr /cache/recovery/boot2
		# trigger blue led (cwm boot)
		busybox echo 0 > ${BOOTREC_LED_RED}
		...
		echo 150 > /sys/class/timed_output/vibrator/enable
		# recovery ramdisk
		load_image=/sbin/ramdisk-cwm.cpio.bz2
		busybox echo "CWM LOADED" >> boot.txt
